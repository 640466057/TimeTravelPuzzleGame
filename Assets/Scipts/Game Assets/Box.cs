...
public class Box : MonoBehaviour
{
    ...
    void Update()
    {
        if (player.TryGetComponent(out PlayerController PC) && gameObject.TryGetComponent(out TimeMachine TM) && Input.GetKeyDown(Settings.keys[1]) && AffectedByTimeTravel && TM.echo.transform.localScale != Vector3.zero)
        {
            TM.TimeTravel();
        }
    }
